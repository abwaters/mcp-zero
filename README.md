...
| **Transport** | Streamable HTTP, stdio | HTTP, SSE, stdio | Streamable HTTP only | stdio (primary) |
| **Authentication** | Okta OAuth2 JWT, OBO token exchange | OAuth 2.0, SAML, SSO (Okta, Azure AD) | Azure Entra ID / OAuth 2.0 | None built-in (commercial plugin) |
| **Governance** | Static YAML/JSON policy files, default-deny, server/tool/user/group rules | RBAC/ABAC, Virtual MCP role-based endpoints | RBAC via Entra ID roles | Plugin-based guardrails, reputation scoring |
...
| **Multi-tenant** | Yes (user/group-level policies) | Yes (role-based endpoints) | Yes (resource-level RBAC) | No (single-user local proxy) |
